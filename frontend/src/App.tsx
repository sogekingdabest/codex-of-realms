...
function RuntimeNotice({ capabilities }: Readonly<{ capabilities: RuntimeCapabilities }>) {
  ...
  const missingModels = [...new Set(
    unavailableCapabilities
      ...
      .map(({ capability }) => capability.model.trim())
      .filter(Boolean),
  )]
  const runtimeUnavailable = unavailableCapabilities.some(
    ({ capability }) => capability.status === 'RUNTIME_UNAVAILABLE',
  )
  const notConfigured = unavailableCapabilities.some(
    ({ capability }) => capability.status === 'NOT_CONFIGURED',
  )
    ...
    <output className="runtime-notice">
      <div>
        ...
        {unavailableCapabilities.map(({ label, capability }) => (
          <span key={label}>
            {label}: {capability.model || 'sin modelo'} ({runtimeStatusLabels[capability.status]}).
          </span>
        ))}
        {runtimeUnavailable && (
          <span>Ollama no responde. Inicia o revisa el runtime local.</span>
        )}
        {notConfigured && (
          <span>Configura el proveedor y el modelo correspondientes antes de continuar.</span>
        )}
      </div>
