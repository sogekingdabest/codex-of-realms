...
The product needs realm management, authorization, document ingestion, vector retrieval, model orchestration, and a structured lore catalogue. These capabilities have different responsibilities, but the MVP has one developer, one deployment target, one transactional data store, and no measured scaling boundary.

Introducing independently deployed services would add network failure modes, distributed observability, contract versioning, deployment coordination, and eventual consistency before the product has validated its core workflow.
...
Use a single Maven module initially. Build-tool modules are not required to prove domain modularity.
...
- Record extraction criteria instead of promising future microservices.
