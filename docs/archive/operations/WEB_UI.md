...
The web application lets users register through Keycloak, invite members, grant spoiler access, ask questions,
read cited sources, and manage the M9 canon catalogue.

## Prerequisites

- Node.js 24 LTS for direct frontend development
- The backend, PostgreSQL, Keycloak, and Ollama dependencies described in [Local development](../../operations/LOCAL_DEVELOPMENT.md)
...
deterministic Java suite. The shared verification command also enforces coverage thresholds and a non-empty LCOV
report. `npm test` remains available as a partial check during development.
