...
M4 and M7 evaluate source-level retrieval against [`demo/evaluation/baseline.json`](../../../demo/evaluation/baseline.json). Tests read the questions and expected source identifiers directly from the dataset. Version 2 expands the baseline to 22 cases over seven original Spanish sources and adds broader public, GM-only, spoiler, restricted, and prompt-injection coverage.
