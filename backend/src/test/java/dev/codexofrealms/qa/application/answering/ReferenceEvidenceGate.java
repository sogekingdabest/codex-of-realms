...
class ReferenceEvidenceGate {
    ...
    ReferenceEvidenceGate(AnsweringProperties properties) {
    ...
    EvidenceGateDecision evaluate(String question, List<RetrievedEvidence> retrieved) {
        ...
        List<RetrievedEvidence> evidence = retrieved.stream()
            ...
            .toList();
        if (evidence.stream().map(RetrievedEvidence::content).anyMatch(ReferenceEvidenceGate::containsInjection)) {
    ...
    EvidenceGateDecision screenPassages(String question, List<RetrievedEvidence> passages) {
        ...
        if (passages.stream().map(RetrievedEvidence::content).anyMatch(ReferenceEvidenceGate::containsInjection)) {
            return EvidenceGateDecision.reject(INDIRECT_INJECTION);
        }
